...
import 'package:kotnew/components/external_dir.dart';
import 'package:kotnew/db_helper.dart';
import 'package:kotnew/model/customer_model.dart';
import 'package:kotnew/model/registration_model.dart';
...
class Controller extends ChangeNotifier {
        ...
        try {
          ...
          if (sof == "1") {
            if (appType == 'ED') {
              ...
              b_url = regModel.c_d![0].baseurl;

              prefs.setString("burl", b_url!);
              // prefs.setString("os", regModel.os!);

              // ignore: avoid_print
              print("cid----bURL-----$cid---$b_url");
              ...
              // await Kotnew.instance.deleteFromTableCommonQuery("companyRegistrationTable", "");
  ...
  getLogin(String userName, String password, BuildContext context) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? burl = prefs.getString("burl");
    print("burl888888888$burl");
    NetConnection.networkConnection(context).then((value) async {
      if (value == true) {
        try {
          Uri url = Uri.parse("${burl}edine/API/login.php");
          Map body = {'uname': userName, 'password': password};
